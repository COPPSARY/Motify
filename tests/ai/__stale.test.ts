...
import { analyzeMotionQuality, buildMotionlyUserMessage } from "../../src/ai/generation-guidance";
...
describe.skipIf(!LIVE)(`stale-layer check on ${MODEL}`, () => {
  for (const [i, prompt] of PROMPTS.entries()) {
    it(`run ${i + 1}`, { timeout: 400000 }, async () => {
      ...
      if (!res.ok) { console.log(`  run ${i + 1}: HTTP ${res.status}`); return; }
      ...
      try {
        const v = validateGeneratedComposition(out, {
          prompt, previousHtml: basis.files["composition.html"],
      ...
      const clears = (out.timelineJs.match(/autoAlpha\s*:\s*0|visibility\s*:\s*["']hidden|display\s*:\s*["']none/g) ?? []).length;

...
import { analyzeMotionQuality, buildMotionlyUserMessage, buildFilmShapeBrief } from "../../src/ai/generation-guidance";
...
describe.skipIf(!LIVE)(`chat request on ${MODEL}`, () => {
  it("generates", { timeout: 400000 }, async () => {
    ...
    const basis = resolveGenerationBasis(blankProjectFiles, {
      id: "blank-composition", duration: 5,
      scenes: [{ id: "main", label: "Main", start: 0, duration: 5, accent: "#7657ff" }],
    });
      ...
      const res = await fetch(
        ...
        { method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            generationConfig: { response_mime_type: "application/json", temperature: 0.65, maxOutputTokens: 65536 },
          }) });
      if (!res.ok) { console.log(`  attempt ${attempt}: HTTP ${res.status}`); await new Promise(r => setTimeout(r, 8000)); continue; }
      const body = await res.json();
      const text = body.candidates?.[0]?.content?.parts?.find((x: { text?: string }) => typeof x.text === "string")?.text;
      const out = repairGeneratedMarkup(parseAiResponseText(text)).result;
      const r = analyzeMotionQuality(out, { prompt: PROMPT });
      console.log(`  score ${r.score} | blocking ${r.blockingIssues.length} | advisory ${r.issues.length - r.blockingIssues.length}`);
      for (const i of r.blockingIssues) console.log("    ! " + i.slice(0, 95));
      console.log(`  title: ${out.title} | ${out.duration}s`);
      console.log(`  scenes: ${(out.scenes ?? []).map(s => s.label).join(" | ")}`);
      console.log(`  html ${out.compositionHtml.length}b | timeline ${out.timelineJs.length}b | hyperframe ${(out.compositionHtml.match(/data-hyperframe-component/g) ?? []).length}`);
      const v = validateGeneratedComposition(out, {
        prompt: PROMPT, previousHtml: basis.files["composition.html"],
        previousDuration: basis.duration, previousScenes: basis.scenes,
        generationProfile: basis.generationProfile,
      });
      console.log(`  RENDERS OK ${v.duration}s warnings: ${JSON.stringify(v.warnings)}`);
      const adapter = createGeneratedAdapterSource({
        id: "dynamic-comp-chat", title: out.title || "Chat", duration: v.duration,
        scenes: v.scenes, width: 1920, height: 1080, fps: 60,
      });
      const files = splitCompositionSource(out.compositionHtml, out.timelineJs, adapter);
      mkdirSync("tests/.render", { recursive: true });
      writeFileSync("tests/.render/draft.json", JSON.stringify({
        version: 1, updatedAt: Date.now(), files, messages: [], assets: [], editorState: {},
        metadata: { title: out.title, duration: v.duration, scenes: v.scenes },
      }));
      writeFileSync("tests/.render/meta.json", JSON.stringify({ title: out.title, duration: v.duration, warnings: v.warnings }));
      return;
    }

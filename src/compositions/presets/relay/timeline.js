...
export function buildRelayTimeline({ root, timeline: t, register }) {
  ...
  const world = get("relayWorld"),
    paper = get("relayPaper");
  const notes = ["Copy", "Sound", "Final"].map((n) => get("relayNote" + n));
  const faces = ["Draft", "Review", "Envelope", "Delivered", "Brand"].map((n) =>
    get("relay" + n + "Face"),
  );
  const [draft, review, envelope, delivered, brand] = faces;
  const stamp = get("relayStamp"),
    tray = get("relayTray");
  const titles = ["Problem", "ReviewTitle", "ApproveTitle", "HandoffTitle"].map(
    (n) => get("relay" + n),
  );
  ...
  place(titles[0], 0, -350);
  place(titles[1], 0, -370, { fontSize: 84 });
  place(titles[2], 0, -345, { fontSize: 88 });
  place(titles[3], 1050, -315, { fontSize: 80 });
  place(paper, 0, 100, { rotationY: -8, rotationX: 4, rotation: -2 });
  place(notes[0], -625, -15, { rotation: -10 });
  place(notes[1], 610, 65, { rotation: 9 });
  place(notes[2], 440, 360, { rotation: -5 });
  place(stamp, 285, 160, { rotation: -16, scale: 1.6 });
  place(tray, 1050, 310, { rotationX: 14 });
  t.set(
    get("relayRoute"),
    { x: -50, y: -280, clipPath: "inset(0 100% 0 0)" },
    0,
  );
  place(get("relayWordmark"), 1050, -20);
  place(get("relayClose"), 1050, 160);
  ...
  t.to(
    paper,
    { rotationY: -4, rotation: -1, duration: 2, ease: "sine.inOut" },
    1.8,
  );
  ...
  exit(draft, 4.35, { y: -28 }, 0.65);
  show(review, 4.65, 0.65);
  words(titles[1], 4.8);
  ...
  exit(titles[2], 13.9, { x: -320 }, 0.7);
  exit(stamp, 14.15, { scale: 0.28, x: 0, y: 0 }, 0.7);
  t.to(
    review,
    {
  ...
  shape(
    {
      ...
      rotationY: -10,
      rotationX: 8,
      backgroundColor: "#e46047",
      borderRadius: 16,
    },
    14.2,
    1.1,
  );
  show(envelope, 14.65, 0.65);
  show(get("relayRoute"), 15.25, 0.45);
  show(tray, 15.6, 0.6);
  t.to(
    get("relayRoute"),
  ...
  exit(envelope, 18.9, { y: 85 }, 0.65);
  show(delivered, 19.25, 0.65);

  // The delivered brief becomes the mark on the same world coordinate.
  exit(titles[3], 20.75, { y: -460 }, 0.65);
  exit(tray, 20.9, { y: 480 }, 0.75);
  exit(delivered, 21.15, { y: 50 }, 0.6);
  shape(
    {
      x: 1050,
      y: -225,
  ...
  show(brand, 21.65, 0.65);
  show(get("relayWordmark"), 22.1, 0.6);
  words(get("relayClose"), 22.65);
  t.to(
    paper,
    { rotationY: 8, y: -234, duration: 2.4, ease: "sine.inOut" },
    23.6,
  );
}
